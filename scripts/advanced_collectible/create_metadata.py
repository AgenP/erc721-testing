...
car_to_image_uri = {
    "Breakout": "ipfs://QmX2dWoJTAogtASA9MHAkKPUbWzjgZC77AmDfkjBgpnRjm",
    "Merc": "ipfs://QmXKRuSHX9iTZbXCkxkH9wn8uGZzcorxDf828pKpUXdDsA",
    "Octane": "ipfs://QmQNeXhuZ9jpkQqfHJnBNMh6ubCXKvtzHB75BbWnx5DybB",
}
...
def write_metadata(token_ids, nft_contract):
    for token_id in range(token_ids):
        ...
        cartype = get_car(nft_contract.tokenIdToCar(token_id))
        metadata_file_name = (
        ...
        if Path(metadata_file_name).exists():
            ...
            # Note, this is currently slightly different to the ipfs uploaded descriptions
            collectible_metadata["description"] = "An Extreme {} Battle Car!".format(
            ...
            image_to_upload = (
                car_to_image_uri[cartype] if not image_to_upload else image_to_upload
            )
...
# Programmatic upload to ipfs
# Alternate: Use IPFS desktop and add manually
def upload_to_ipfs(filepath):
    # Opening the image/metadata file in binary and setting it to fp (filepath)
